# Sources and demo links used

The website is an educational, non-surgical exercise guide. It uses conservative, low-impact, progressive exercise principles and links to trusted public medical/arthritis sources for demonstrations and context.

## Exercise demos and clinical guidance

- AAOS OrthoInfo — Knee Conditioning Program: https://orthoinfo.aaos.org/en/recovery/knee-conditioning-program/
- AAOS OrthoInfo — Knee Exercises: https://orthoinfo.aaos.org/en/staying-healthy/knee-exercises/
- AAOS OrthoInfo — Hip Conditioning Program: https://orthoinfo.aaos.org/en/recovery/hip-conditioning-program/
- NHS — Exercises for sciatica problems: https://www.nhs.uk/live-well/exercise/exercises-sciatica-problems/
- NICE Guideline NG59 — Low back pain and sciatica in over 16s: https://www.nice.org.uk/guidance/ng59
- Arthritis Foundation — Benefits of Exercise for Osteoarthritis: https://www.arthritis.org/health-wellness/healthy-living/physical-activity/getting-started/benefits-of-exercise-for-osteoarthritis
- Arthritis Foundation — OA diagnosis: why weight loss matters: https://www.arthritis.org/health-wellness/healthy-living/nutrition/weight-loss/oa-diagnosis-why-weight-loss-matters

## Design/content principles

1. Treat cartilage issues as joint-load problems, not just tissue-name problems.
2. Separate joint patterns from nerve/sciatica patterns using a simple pain map.
3. Prefer low-impact cardio for overweight patients: low-impact indoor marching, stationary cycling, elliptical, and flat walking.
4. Strengthen the support system around joints: quadriceps, glutes, hips, hamstrings, calves, feet/ankles, and core.
5. Progress based on symptoms and next-day response.
6. Avoid shame-based weight language; frame weight loss as joint-load reduction if relevant.
7. Make red flags obvious.
